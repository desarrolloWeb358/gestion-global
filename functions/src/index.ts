...
// Reutilizado para scraping Rama Judicial (CPNU + TYBA)
export { consultarRadicado as helloWorld } from "./ramaJudicial/consultarRadicado";
...
export const crearUsuarioDesdeAdmin = onRequest(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      if (req.body.action === "toggleActivo") {
        const { uid, activo } = req.body as { uid?: string; activo?: boolean };
...
export const recordatorioPlazosLegales = onSchedule(
  ...
  async () => {
    ...
    async function notificarDestinatario(
      ...
      const { urgencia, colorUrgencia, tipoLabel, titulo, nombreCliente, fechaLimiteStr, ruta, subject } = payload;
            ...
            <tr><td align="center">
              <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 10px 15px rgba(0,0,0,0.05);">
                ...
                <tr><td style="padding:24px;">
                  <p style="margin-top:0;">Hola <strong>${nombreDestinatario}</strong>,</p>
      ...
      await db.collection(`usuarios/${uid}/notificaciones`).add({
    ...
    for (const docSnap of snap.docs) {
      try {
        ...
        const payload = { urgencia, colorUrgencia, tipoLabel, titulo, nombreCliente, fechaLimiteStr, ruta, subject };

        // Notificar al abogado principal
        await notificarDestinatario(abogadoId, nombreAbogado, correoAbogado, payload);
        logger.info(`[recordatorioPlazosLegales] Notificado abogado ${abogadoId} para valor ${valorId}`);

        // Notificar a delegados (ej: Lina) definidos en el doc del abogado
        const delegados: string[] = abogadoData?.delegadosNotificaciones ?? [];
        for (const delegadoId of delegados) {
          const dSnap = await db.collection("usuarios").doc(delegadoId).get();
          if (!dSnap.exists) continue;
          const dData = dSnap.data() as any;
          if (!dData?.email) continue;
          await notificarDestinatario(delegadoId, dData.nombre || "Colaborador", dData.email, payload);
          logger.info(`[recordatorioPlazosLegales] Notificado delegado ${delegadoId} para valor ${valorId}`);
        }
      } catch (err) {
